...
export async function ensureCodeModeRepoSetup(cwd, cronExpression = DEFAULT_CODE_MODE_CRON) {
    await writeCodeModeWorkflow(cwd, cronExpression);
    await writeCodeModeAgentSnippets(cwd);
}
async function writeCodeModeWorkflow(cwd, cronExpression) {
...
async function writeCodeModeAgentSnippets(cwd) {
    ...
    await Promise.all(CODE_MODE_AGENT_FILES.map((fileName) => writeCodeModeAgentSnippet(path.join(cwd, fileName), snippet)));
}
async function writeCodeModeAgentSnippet(agentsPath, snippet) {
    let currentContent = "";
    try {
    ...
    const nextContent = startIndex !== -1 && endIndex !== -1 && endIndex > startIndex
...
function createCodeModeWorkflow(cronExpression) {
