...
export function normalizeWriteTodosToolArgs(args) {
        ...
        wrapToolCall: async (request, handler) => {
            const normalizedRequest = request.toolCall.name === WRITE_TODOS_TOOL_NAME
                ? {
                    ...
                    toolCall: {
                        ...request.toolCall,
                        args: normalizeWriteTodosToolArgs(request.toolCall.args),
                    },
                }
                : request;
            try {
                ...
                if (ToolMessage.isInstance(result)) {
                    return sanitizeUnsupportedToolMessageContent(result, normalizedRequest.toolCall);
                }
                return result;
            }
            catch (error) {
...
function isToolSchemaValidationError(error) {
    return (error instanceof Error && error.message.includes(TOOL_SCHEMA_ERROR_MESSAGE));
}
function createToolSchemaErrorMessage(toolCall, error) {
    const message = error instanceof Error ? error.message : "Unknown tool schema error.";
    const guidance = toolCall.name === "edit_file"
...
function sanitizeUnsupportedToolMessageContent(message, toolCall) {
    ...
    const mediaTypes = [
        ...new Set(unsupportedBlocks
            .map((block) => getContentBlockMediaType(block))
            .filter((mediaType) => Boolean(mediaType))),
    ];
    return new ToolMessage({
...
function getUnsupportedContentBlocks(content) {
    if (!Array.isArray(content)) {
        return [];
    }
    return content.filter(isUnsupportedContentBlock);
}
function isUnsupportedContentBlock(block) {
    if (!isRecord(block)) {
        return false;
    }
