...
async function ingest(options = {}) {
    ...
    const config = await readConnectorConfig("x", {
        enabled: false,
        listIds: [],
        maxPagesPerStream: 2,
    ...
    if (!config.enabled) {
        return {
            connectorId: "x",
            message: "X connector is not enabled. Configure ~/.openwiki/connectors/x/config.json and set OPENWIKI_X_ACCESS_TOKEN in ~/.openwiki/.env.",
            rawFiles,
            runId,
...
function isXStream(value) {
    return DEFAULT_STREAMS.includes(value);
}
