...
async function ingest(options = {}) {
    ...
    if (config.repos.length === 0) {
        return {
            ...
            message: "No local repositories configured. Add repos to ~/.openwiki/connectors/git-repo/config.json.",
            rawFiles,
            runId,
...
async function createRepoManifest(id, repoPath) {
    ...
    const changedFiles = (await runGit(repoPath, ["diff", "--name-status", "HEAD"]))
        .split(/\r?\n/u)
        .filter(Boolean);
    return {
        branch,
